...
if ($Help) {
    ...
    Write-Host "此脚本将依次执行: Lint 静态分析 → 本地单元测试 → 视觉回归测试(VRT) → 仪器化 UI 测试。"
...
Write-Host "[1/4] 开始执行 Lint 静态代码分析..." -ForegroundColor Yellow
.\gradlew lintDebug

$LintExit = $LASTEXITCODE

Write-Host ""
Write-Host "[2/4] 开始执行本地单元测试 (Unit Tests)..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[3/4] 开始执行视觉回归测试 (VRT / Roborazzi)..." -ForegroundColor Yellow
.\gradlew verifyRoborazziDebug --continue
$VrtExit = $LASTEXITCODE

Write-Host ""
Write-Host "[4/4] 开始执行仪器化测试 (Instrumented Tests)..." -ForegroundColor Yellow
...
if ($VrtExit -ne 0) {
    Write-Host "❌ 视觉回归测试(VRT)发现 UI 快照不匹配，请查看 app/build/reports/roborazzi/。" -ForegroundColor Red
}
else {
    Write-Host "✅ 视觉回归测试(VRT)全部通过！" -ForegroundColor Green
}

if ($LintExit -eq 0 -and $UnitTestExit -eq 0 -and $VrtExit -eq 0 -and $InstrumentedTestExit -eq 0 -and $UnitTestValid -and $InstrumentedTestValid) {
    Write-Host ""
